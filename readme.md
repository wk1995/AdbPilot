...
- 悬浮窗模式：简洁置顶状态条，显示已连接设备名、连接类型和状态，多设备时展示所有在线设备摘要。
...
GUI 会每隔数秒自动刷新设备列表。设备断开后会显示为“已断开”，当前设备选择会被清空，避免继续对已断开的设备执行安装、日志、文件传输等操作。悬浮窗模式下，如果监听到设备断开，会显示“警示：设备已断开”。悬浮窗所有操作通过右键菜单触发，双击球体可回到主界面。锁定设备后，即使该设备断开也不会自动解除锁定，只有点击“解除锁定”才会恢复普通设备选择逻辑。
